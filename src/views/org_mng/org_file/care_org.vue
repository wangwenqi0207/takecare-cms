  ...
  <div class="comment_box" style="padding:0px;margin-top:0px;margin-left:0px;padding-bottom:10px;padding-top:15px;">
    ...
    <div class="tableList" style="height:83%">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <p v-if="scope.row.organizationType=='0'">敬老院</p>
            <p v-else-if="scope.row.organizationType=='1'">福利院</p>
            <p v-else-if="scope.row.organizationType=='2'">养老院</p>
            <p v-else-if="scope.row.organizationType=='3'">老年公寓</p>
            <p v-else-if="scope.row.organizationType=='4'">护理院</p>
            <p v-else-if="scope.row.organizationType=='5'">医院</p>
            <p v-else-if="scope.row.organizationType=='6'">社区卫生服务中心</p>
            <p v-else-if="scope.row.organizationType=='7'">私营公司</p>
            <p v-else>{{ scope.row.orgNature }}</p>
          </template>
        </el-table-column>
        <el-table-column
          prop="serviceType"
        ...
        >
          <template slot-scope="scope">
            <p v-for="(item,index) in scope.row.serviceType" :key="index">
              <span v-if="item=='0'">&nbsp;居家生活照料&nbsp;</span>
              <span v-else-if="item=='1'">&nbsp;居家医疗护理&nbsp;</span>
              <span v-else-if="item=='2'">&nbsp;机构生活照料&nbsp;</span>
              <span v-else-if="item=='3'">&nbsp;机构医疗护理&nbsp;</span>
        ...
        >
          <template slot-scope="scope">
            <p v-for="(item,index) in scope.row.designatedAgencie" :key="index">
              <span v-if="item=='0'">&nbsp;医保定点&nbsp;</span>
              <span v-else-if="item=='1'">&nbsp;长护定点&nbsp;</span>
              <span v-else-if="item=='2'">&nbsp;民政定点&nbsp;</span>
              <span v-else-if="item=='3'">&nbsp;残联定点&nbsp;</span>
              <span v-else-if="item=='4'">&nbsp;卫健委定点&nbsp;</span>
              <span v-else>&nbsp;{{ item }}&nbsp;</span>
            </p>
          </template>
        </el-table-column>
...
export default {
  ...
  mounted() {
    this.resourcePermission = permissionData('护理机构')
  },
  methods: {
    getCityData() {
    ...
    write_org() {
      if (this.multipleSelection && this.multipleSelection.length === 1) {
        ...
        // console.log(org_id)
        this.$router.push({
          path: '/org_mng/care_org/write_org',
          query: {
            id: org_id
          }
        })
    ...
    // 新增机构
    add_org() {
      this.$router.push('/org_mng/care_org/add_org')
    },
    // 查看个人资料  路由跳转 传参
    handleEdit(id) {
      this.$router.push({
        path: '/org_mng/care_org/look_org',
        query: {
          id: id
        }
      })
