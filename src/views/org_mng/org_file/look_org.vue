  ...
  <div id="scrollBox" class="add_org" style="overflow:auto;height:100%">
    <sticky :z-index="10" class-name="sub-navbar" :sticky-top="130">
      <div class="add_org_nav">
        ...
        <div class="add_org_nav_right">
          <el-checkbox v-model="promitLicense" class="add_org_nav_right_checkbox" disabled />
          <p>立即启用</p>
        </div>
      </div>
    </sticky>
    ...
    >
            ...
            <el-form-item label="缩略图" required prop="listPicture">
              ...
              <div class="click_upimg">
                ...
                >
                  ...
                  />
                  <div v-if="ruleForm.listPicture" class="slotBox" style="width:120px;height:120px;">
                    <img
                  ...
                  <div @click="getImageTypeIndex(index)">
                    ...
                    >
                      ...
                      <i
                        ...
                        class="el-icon-plus avatar-uploader-icon"
                      />
                      <div v-if="item.url" class="slotBox" style="width:120px;height:120px;">
                        <img
                          class="el-upload-list__item-thumbnail"
                          :src="item.url"
                          alt
      ...
      <!-- <div class="org_submit">
        <el-button class="org_submit_btn" size="medium" @click="colse_off">返回</el-button>
      </div> -->
    </el-form>
    <div class="org_submit">
      <el-button class="org_submit_btn" size="medium" @click="colse_off">返回</el-button>
      <!-- <el-button type="primary" class="org_submit_btn" @click="add_submit()">提交</el-button> -->
    </div>
  </div>
</template>
...
import { orgInfo } from '@/api/organization'
    ...
    hintGetFun() {
      orgInfo({ id: this.$route.query.id }).then(res => {
        if (res) {
          ...
          this.ruleForm.serviceType = result.serviceType.map(function(value, index, array) {
            // code
            return value + ''
          })
          // console.log(this.ruleForm.serviceType)
          this.ruleForm.designatedAgencie = result.designatedAgencie.map(function(value, index, array) {
            // code
            return value + ''
          })
          this.ruleForm.organizationStatus = result.organizationStatus + ''
          this.ruleForm.organizationType = result.organizationType + ''
          if (result.organizationStatus == 1) {
            this.promitLicense = true
          } else {
            this.promitLicense = false
          }
    ...
    fnUploadRequestSLT(option) {
      ...
      client.multipartUpload(fileNames, option.file, {
        ...
        if (val.res.statusCode === 200) {
          ...
          // this.$set(this.ruleForm,'listPicture','http://ecare-test.oss-cn-hangzhou.aliyuncs.com/' + val.name)
          this.$forceUpdate()
          // console.log(this.ruleForm.listPicture)
          return val
        } else {
          option.onError('上传失败')
        }
