  ...
  <div style="height:100%;">
    ...
    <div class="search_bar">
      <el-form :inline="true" :model="searchForm" class="demo-form-inline">
        ...
        <el-form-item v-if="tabId == 'first'">
          <el-dropdown trigger="click" class="rela" @command="handleCommandStatus">
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item command="0&待支付">待支付</el-dropdown-item>
              <el-dropdown-item command="1&待派单">待派单</el-dropdown-item>
              <el-dropdown-item command="2&待派护理员/待发货/待入住/待服务">待派护理员/待发货/待入住/待服务</el-dropdown-item>
        ...
        <el-form-item v-if="tabId != 'second'" label="支付日期">
          <el-date-picker
        ...
        <tbody v-for="item in tableData" :key="item.id">
            ...
            <td colspan="8">
              <div class="fl_box" style="width:100%">
                <div class="flt mar_right_40 mar_left_20">
                  <el-checkbox :name="item.id+'&&'+item.actualTotalPrice+'&&'+item.paymentStatus+'&&'+item.orderStatus" />
                </div>
          ...
          <tr v-for="(orderItem,index) in item.orderItems" :key="index">
            ...
            <td v-if="index==0">
              ...
              <div>
                <span>￥{{ orderItem.price+'*'+orderItem.counts }}</span>
              </div>
            </td>
            ...
            <td v-if="index==0" :rowspan="item.orderItems.length">
              {{ paymentStatusMap[item.paymentStatus] }}
            </td>
            <td v-if="index==0" :rowspan="item.orderItems.length">
              ...
              <el-button v-if="resourcePermission['派单']&&item.paymentStatus==2&&item.orderStatus==1" type="primary" size="mini" @click="PDAction(item.id)">派单</el-button>
              <el-button v-else-if="resourcePermission['确认退单']&&item.paymentStatus==4&&item.orderStatus==5" type="primary" size="mini" @click="TDAction(item.id,item)">确认退单</el-button>
              <el-button v-else-if="resourcePermission['派护理员']&&item.paymentStatus==2&&item.orderStatus==2&&(item.type==0||item.type==1||item.type==5||item.type==6)" type="primary" size="mini" @click="HLYAction(item.id,item.type)">派护理员</el-button>
              <el-button v-else-if="resourcePermission['入住']&&item.paymentStatus==2&&item.orderStatus==2&&item.type==2" type="primary" size="mini" @click="RZAction(item.id)">入住</el-button>
              <el-button v-else-if="resourcePermission['发货物流']&&item.paymentStatus==2&&item.orderStatus==2&&(item.type==3||item.type==4)" type="primary" size="mini" @click="FHAction(item.id)">发货物流</el-button>
    ...
    <el-dialog title="派单" :visible.sync="dialogFormVisible" width="600px">
      <el-form ref="form" :model="form" :rules="rules">
        <el-form-item label="派单给：" :label-width="formLabelWidth" prop="organizationId">
          <el-select v-model="form.organizationId" style="width:80%;" placeholder="请选择机构">
            <el-option
        ...
        <el-form-item label="搜索机构：" :label-width="formLabelWidth" prop="reason">
          <el-input v-model.trim="form.orgName" autocomplete="off" style="width:80%;" placeholder="请输入机构名称">
            <el-button slot="append" icon="el-icon-search" @click="searchORG" />
          </el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer" style="text-align:center;">
    ...
    <el-dialog title="派护理员" :visible.sync="dialogFormVisibleHLY" width="600px">
      <el-form ref="formHLY" :model="formHLY" :rules="rulesHLY">
        <el-form-item label="护理员：" :label-width="formLabelWidth" prop="StaffId">
          <el-select v-model="formHLY.StaffId" style="width:80%;" placeholder="请选择护理员">
            <el-option
              v-for="(item,index) in nursingAllList"
              :key="index"
        ...
        <el-form-item label="搜索护理员：" :label-width="formLabelWidth" prop="reason">
          <el-input v-model.trim="formHLY.staffName" autocomplete="off" style="width:80%;" placeholder="请输入护理员名字">
            <el-button slot="append" icon="el-icon-search" @click="searchHLY" />
          </el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer" style="text-align:center;">
    ...
    <el-dialog title="入住" :visible.sync="dialogFormVisibleRZ" width="600px">
      <el-form ref="formRZ" :model="formRZ" :rules="rulesRZ">
        ...
        <!-- <el-form-item label="入住时间：" :label-width="formLabelWidth" prop="RZTime">
          <el-date-picker
            v-model="formRZ.RZTime"
            type="datetime"
            placeholder="选择入住时间">
          </el-date-picker>
        </el-form-item> -->
    ...
    <el-dialog title="取消订单" :visible.sync="dialogFormVisibleQXDD" width="600px">
      <el-form ref="formQXDD" :model="formQXDD" :rules="rulesQXDD">
        ...
        <el-form-item v-if="maxMoney!=0" label="申请退款金额：" label-width="180px" prop="money">
          <el-input-number v-model="formQXDD.money" controls-position="right" :min="0.01" :max="maxMoney" step="0.01" />
        </el-form-item>
        <el-form-item v-else label="申请退款金额：" label-width="180px" prop="money">
          <el-input-number v-model="formQXDD.money" controls-position="right" :min="0" :max="maxMoney" step="0.01" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer" style="text-align:center;">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rulesRZ: {
        ...
        // RZTime: [{ required: true, message: '请填写入住时间', trigger: 'blur' }
        // ]
      },
      // 确认退单
  ...
  mounted() {
    this.offset = offset()
    this.resourcePermission = permissionData('全部订单')
    // 定时器定时刷新页面
    // if (this.timer) {
  ...
  methods: {
    ...
    checkDetail(type, id) {
      ...
      if (type === 0) {
        this.$router.push({ path: `/order_mng/all_order/daily_order_detail`, query: { id: id }})
      } else if (type === 1) {
        this.$router.push({ path: `/order_mng/all_order/medical_order_detail`, query: { id: id }})
      } else if (type === 2) {
        this.$router.push({ path: `/order_mng/all_order/org_order_detail`, query: { id: id }})
      } else if (type === 3) {
        this.$router.push({ path: `/order_mng/all_order/care_equipment_order_detail`, query: { id: id, type: 1 }})
      } else if (type === 4) {
        this.$router.push({ path: `/order_mng/all_order/care_equipment_order_detail`, query: { id: id, type: 2 }})
      } else if (type === 5) {
        this.$router.push({ path: `/order_mng/all_order/standard_order_detail`, query: { id: id, type: 1 }})
      } else if (type === 6) {
        this.$router.push({ path: `/order_mng/all_order/custom_order_detail`, query: { id: id, type: 2 }})
      } else if (type === 7) {
        this.$router.push({ path: `/order_mng/all_order/long_care_order_detail`, query: { id: id }})
      }
    ...
    onSubmitQXDD() {
      ...
      // 退款金额小于0或大于最大金额
      // if (this.formQXDD.money < 0 || this.formQXDD.money > this.maxMoney) {
      //   this.$message({
      //     type: 'warning',
      //     message: '请正确填写退款金额'
      //   })
        ...
        if (valid) {
          ...
          cancelOrder6002(obj).then(res => {
            if (res) {
              ...
              this.searchData()
              this.getAllOrder([], 'first')
              this.getAllOrder([{ 'key': 'OrderStatus', 'value': '0' }], 'second')
              this.getAllOrder([{ 'key': 'OrderStatus', 'value': '1' }], 'third')
              this.getAllOrder([{ 'key': 'OrderStatus', 'value': '5' }], 'seven')
              this.getAllOrder([{ 'key': 'OrderStatus', 'value': '6' }], 'eight')
            } else {
              this.$message({
                type: 'warning',
                message: '取消失败!'
    ...
    checkIDs() {
      if ($('#' + this.tabId + ' :input:checkbox:checked').length === 1) {
        var currentData = $('#' + this.tabId + ' :input:checkbox:checked')[0].name.split('&&')
        // console.log(currentData)
        var paymentStatus = currentData[2]
        var orderStatus = currentData[3]
        if (paymentStatus < 2) {
        ...
        } else if (paymentStatus == '2' && orderStatus != '4' && orderStatus != '5' && orderStatus != '6') {
          this.$confirm('是否确定取消订单?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '再想想',
            type: 'warning'
    ...
    // 搜索护理员
    searchHLY() {
      console.log(this.formHLY.staffName)
      const data = {
        'numberPerPage': this.formHLY.staffName ? 1000 : 15,
        'currentPage': 1
      }
      var obj = []
      if (this.currentType == 0) {
        obj = [{ 'key': 'Type', 'value': '0' }]
      } else if (this.currentType == 1) {
        obj = [{ 'filterLogic': 1, 'key': 'Type', 'value': '1' }, { 'filterLogic': 1, 'key': 'Type', 'value': '2' }]
      }
      if (this.formHLY.staffName) {
        var obj1 = {
          'key': 'FullName',
          'filterType': 5,
          'value': this.formHLY.staffName
        }
        obj.push(obj1)
      }
      data.filters = obj
      staffSearch(data).then(res => {
        if (res) {
          this.$emit('updateHLYData', { data: res.data, flag: 2 })
    ...
    searchORG() {
      ...
      const data = {
        'currentPage': 1,
        'numberPerPage': this.form.orgName ? 1000 : 15,
        // 'account': this.searchForm.account,
        filters: [{ key: 'Name', value: this.form.orgName, filterType: 5 }]
        // 'idNo': this.searchForm.idNo,
        // 'empno': this.searchForm.empno,
        // 'orgId': this.searchForm.orgId
      }
      orgSearch(data).then(res => {
        if (res) {
          this.$emit('updateOrgData', { data: res.data, flag: 2 })
    ...
    PDAction(val) {
      ...
      orgSearch(data).then(res => {
        if (res) {
          this.$emit('updateOrgData', { data: res.data, flag: 1 })
          this.form.organizationId = ''
        } else {
          this.$message({
            message: '获取列表失败',
    ...
    HLYAction(val, type) {
      ...
      staffSearch(data).then(res => {
        if (res) {
          this.$emit('updateHLYData', { data: res.data, flag: 1 })
          this.formHLY.StaffId = ''
        } else {
          this.$message({
            message: '获取数据失败',
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          assignOrgToService6002(dataObj).then(res => {
            ...
            this.getAllOrder([], 'first')
            this.getAllOrder([{ 'key': 'OrderStatus', 'value': '2' }], 'fourth')
            this.getAllOrder([{ 'key': 'OrderStatus', 'value': '1' }], 'third')
        ...
        if (valid) {
          ...
          for (var item of this.nursingAllList) {
            if (item.userId == this.formHLY.StaffId) {
              ...
              data.staffName = item.lastName + item.firstName
              data.staffGender = item.sex
              data.staffPhoto = item.profilePicture
              data.staffPhoneNumber = item.phoneNumber
              data.staffIDCard = item.identityCard
              data.staffType = item.type
              data.staffCertificate = item.qualificationCertificateType
            }
          }
          // assignOrgToService6002
          var dataObj = {
          ...
          assignStaff6002(dataObj).then(res => {
            ...
            this.getAllOrder([], 'first')
            this.getAllOrder([{ 'key': 'OrderStatus', 'value': '3' }], 'five')
            this.getAllOrder([{ 'key': 'OrderStatus', 'value': '2' }], 'fourth')
