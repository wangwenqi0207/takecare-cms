  ...
  <div style="height:100%;">
    ...
    <div class="search_bar">
      <el-form :inline="true" :model="searchForm" class="demo-form-inline">
        ...
        <el-form-item label>
          <el-input v-model="searchForm.order_no" class="searchInput_300" :placeholder="placeholder" clearable>
...
export default {
  ...
  mounted() {
    this.resourcePermission = permissionData('生活照料订单')
    this.offset = offset()
  },
  methods: {
    ...
    checkDetail(type, id) {
      ...
      if (type === 0) {
        this.$router.push({ path: `/order_mng/daily_care/order_detail`, query: { id: id }})
      } else if (type === 1) {
        this.$router.push({ path: `/order_mng/medical_care/order_detail`, query: { id: id }})
      } else if (type === 2) {
        this.$router.push({ path: `/order_mng/organization_care/order_detail`, query: { id: id }})
      } else if (type === 3) {
        this.$router.push({ path: `/order_mng/care_equipment/order_detail`, query: { id: id, type: 1 }})
      } else if (type === 4) {
        this.$router.push({ path: `/order_mng/care_equipment/order_detail`, query: { id: id, type: 2 }})
      } else if (type === 5) {
        this.$router.push({ path: `/order_mng/community_order/order_detail`, query: { id: id, type: 1 }})
      } else if (type === 6) {
        this.$router.push({ path: `/order_mng/community_order/custom_order_detail`, query: { id: id, type: 2 }})
      } else if (type === 7) {
        ...
        if (valid) {
          ...
          cancelOrder6002(obj).then(res => {
            if (res) {
              ...
              this.searchData()
              this.getAllOrder([{ 'key': 'Type', 'value': '0' }], 'first')
              this.getAllOrder([{ 'key': 'Type', 'value': '0' }, { 'key': 'OrderStatus', 'value': '0' }], 'second')
              this.getAllOrder([{ 'key': 'Type', 'value': '0' }, { 'key': 'OrderStatus', 'value': '1' }], 'third')
              this.getAllOrder([{ 'key': 'Type', 'value': '0' }, { 'key': 'OrderStatus', 'value': '5' }], 'seven')
              this.getAllOrder([{ 'key': 'Type', 'value': '0' }, { 'key': 'OrderStatus', 'value': '6' }], 'eight')
            } else {
              this.$message({
                type: 'warning',
                message: '取消失败!'
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          assignOrgToService6002(dataObj).then(res => {
            ...
            this.getAllOrder([{ 'key': 'Type', 'value': '0' }], 'first')
            this.getAllOrder([{ 'key': 'Type', 'value': '0' }, { 'key': 'OrderStatus', 'value': '2' }], 'fourth')
            this.getAllOrder([{ 'key': 'Type', 'value': '0' }, { 'key': 'OrderStatus', 'value': '1' }], 'third')
      ...
      this.$refs.formHLY.validate((valid) => {
        if (valid) {
          ...
          assignStaff6002(dataObj).then(res => {
            ...
            this.getAllOrder([{ 'key': 'Type', 'value': '0' }], 'first')
            this.getAllOrder([{ 'key': 'Type', 'value': '0' }, { 'key': 'OrderStatus', 'value': '3' }], 'five')
            this.getAllOrder([{ 'key': 'Type', 'value': '0' }, { 'key': 'OrderStatus', 'value': '2' }], 'fourth')
