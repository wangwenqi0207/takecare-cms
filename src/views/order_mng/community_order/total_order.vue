...
export default {
  ...
  mounted() {
    this.resourcePermission = permissionData('社区订单')
    this.offset = offset()
  },
  methods: {
    ...
    getAllOrder(filtersData, id) {
      orderSearch({ 'currentPage': 1, 'numberPerPage': 10, 'sortList': this.sortList, 'filters': filtersData, 'orderTypes': [6, 5] }).then(res => {
        var obj = {}
        obj.data = res.data
        obj.id = id
        obj.totalItems = res.pager.totalItems
