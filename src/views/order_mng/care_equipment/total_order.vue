...
export default {
  ...
  mounted() {
    this.resourcePermission = permissionData('护理器材订单')
    this.offset = offset()
  },
  methods: {
    ...
    getAllOrder(filtersData, id) {
      orderSearch({ 'currentPage': 1, 'numberPerPage': 10, 'sortList': this.sortList, 'filters': filtersData, 'orderTypes': [4, 3] }).then(res => {
        var obj = {}
        obj.data = res.data
        obj.id = id
        obj.totalItems = res.pager.totalItems
    ...
    searchORG() {
      ...
      //     this.$emit('updateHLYData', res.data.records)
