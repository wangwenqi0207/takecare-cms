      ...
      <table border="1" cellpadding="0" cellspacing="0" border-collapse="collapse" class="table">
        ...
        <tbody v-for="item in tableData" :key="item.id">
          ...
          <tr v-for="(orderItem,index) in item.orderItems" :key="index">
            ...
            <td v-if="index==0" :rowspan="item.orderItems.length">
              <!-- <el-button v-if="item.orderStatus=='0'" type="primary" size="mini" @click="QRAction(item.id)">确认订单</el-button>
              <el-button v-else-if="item.orderStatus=='1'" type="primary" size="mini" @click="PDAction(item.id)">派单</el-button>
              <el-button v-else-if="item.orderStatus=='5'" type="primary" size="mini" @click="TDAction(item.id)">确认退单</el-button>
              <el-button v-else-if="item.orderStatus=='2'&&(item.type==0||item.type==1||item.type==5||item.type==6)" type="primary" size="mini" @click="HLYAction(item.id)">派护理员</el-button>
              <el-button v-else-if="item.orderStatus=='2'&&item.type==2" type="primary" size="mini" @click="RZAction(item.id)">入住</el-button>
              <el-button v-else-if="item.orderStatus=='2'&&(item.type==3||item.type==4)" type="primary" size="mini" @click="FHAction(item.id)">发货物流</el-button> -->
