<template>
  <div id="allOrder" class="page_container" style="overflow:auto;height:100%">
    <el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane label="全部" name="first" style="height:100%;">
        <template slot-scope="scope" style="height:100%;">
          <div :class="scope" style="height:100%;">
            <totalOrder :table-data="tab1" :org-list="orgList" :nursing-all-list="nursingAllList" tab-id="first" style="height:100%;" :total-items1="totalItems1" @updateData="updateData" @updateHLYData="updateHLYData" @updateOrgData="updateOrgData" @handleCurrentChange="handleCurrentChange">
      ...
      <el-tab-pane label="待派护理员/待发货/待入住/待护理" name="fourth" style="height:100%;">
        <template slot-scope="scope" style="height:100%;">
          <div :class="scope" style="height:100%;">
            <totalOrder :table-data="tab4" :nursing-all-list="nursingAllList" tab-id="fourth" style="height:100%;" :current-page1="currentPage4" :total-items1="totalItems4" @updateHLYData="updateHLYData" @updateData="updateData" @handleCurrentChange="handleCurrentChange">
              <!-- <template v-slot:default="slotProps">
                {{ slotProps.user.age }}
              </template> -->
              <!-- <template v-slot="{user = {sex:'nv'}}">
...
export default {
  ...
  methods: {
    apiDataTest() {
      // 长护险接口调试
      orderSearch({ 'currentPage': 1, 'numberPerPage': 10, 'sortList': this.sortList }).then(res => {
        this.tab1 = res.data
        this.currentPage1 = 1
        this.totalItems1 = res.pager.totalItems
        console.log(res)
      })
      orderSearch({ 'currentPage': 1, 'numberPerPage': 10, 'sortList': this.sortList, 'filters': [{ 'key': 'OrderStatus', 'value': '0' }] }).then(res => {
        this.tab2 = res.data
        this.currentPage2 = 1
        this.totalItems2 = res.pager.totalItems
        console.log(res)
      })
      orderSearch({ 'currentPage': 1, 'numberPerPage': 10, 'sortList': this.sortList, 'filters': [{ 'key': 'OrderStatus', 'value': '1' }] }).then(res => {
        this.tab3 = res.data
        this.currentPage3 = 1
        this.totalItems3 = res.pager.totalItems
        console.log(res)
      })
      orderSearch({ 'currentPage': 1, 'numberPerPage': 10, 'sortList': this.sortList, 'filters': [{ 'key': 'OrderStatus', 'value': '2' }] }).then(res => {
        this.tab4 = res.data
        this.currentPage4 = 1
        this.totalItems4 = res.pager.totalItems
        console.log(res)
      })
      orderSearch({ 'currentPage': 1, 'numberPerPage': 10, 'sortList': this.sortList, 'filters': [{ 'key': 'OrderStatus', 'value': '3' }] }).then(res => {
        this.tab5 = res.data
        this.currentPage5 = 1
        this.totalItems5 = res.pager.totalItems
        console.log(res)
      })
      orderSearch({ 'currentPage': 1, 'numberPerPage': 10, 'sortList': this.sortList, 'filters': [{ 'key': 'OrderStatus', 'value': '4' }] }).then(res => {
        this.tab6 = res.data
        this.currentPage6 = 1
        this.totalItems6 = res.pager.totalItems
        console.log(res)
      })
      orderSearch({ 'currentPage': 1, 'numberPerPage': 10, 'sortList': this.sortList, 'filters': [{ 'key': 'OrderStatus', 'value': '5' }] }).then(res => {
        this.tab7 = res.data
        this.currentPage7 = 1
        this.totalItems7 = res.pager.totalItems
        console.log(res)
      })
      orderSearch({ 'currentPage': 1, 'numberPerPage': 10, 'sortList': this.sortList, 'filters': [{ 'key': 'OrderStatus', 'value': '6' }] }).then(res => {
        this.tab8 = res.data
        this.currentPage8 = 1
      ...
      // 获取所有机构列表
      orgSearch({ 'numberPerPage': 15, 'currentPage': 1 }).then(res => {
        if (res) {
      ...
      // 获取所有护理员
      staffSearch({ 'numberPerPage': 15, 'currentPage': 1 }).then(res => {
        if (res) {
    ...
    calculateClick() {
      ...
      const clickMap = {
        first: function(res) {
          that.tab1 = res.data
          that.totalItems1 = res.totalItems
          return
        },
        second: function(res) {
          console.log('click-second')
          that.tab2 = res.data
          that.totalItems2 = res.totalItems
          return 0
        },
        third: function(res) {
          console.log('click-third')
          that.tab3 = res.data
          that.totalItems3 = res.totalItems
          return 0
        }
      }
      return {
        clickClac: function(clickType, val) {
          return clickMap[clickType] && clickMap[clickType](val)
        },
        addStrategy: function(clickType, fn) { // 注册新计算方式
          if (clickMap[clickType]) return
          clickMap[clickType] = fn
        }
      }
    },
    handleCurrentChange(res) {
      console.log('888', res.id)
      // this.calculateClick().clickClac(res.id, res)
      if (res.id === 'first') {
        this.tab1 = res.data
        this.totalItems1 = res.totalItems
      } else if (res.id === 'second') {
