        ...
        <tbody v-for="item in tableData" :key="item.id">
          ...
          <tr v-for="(orderItem,index) in item.orderItems" :key="index">
            ...
            <td v-if="index==0" :rowspan="item.orderItems.length">
              ...
              <el-button v-else-if="resourcePermission['派护理员']&&item.orderStatus=='2'&&(item.type==0||item.type==1||item.type==5||item.type==6)" type="primary" size="mini" @click="HLYAction(item.id,item.type)">派护理员</el-button>
