<template>
  <div id="allOrder" class="page_container" style="overflow:auto;height:100%">
    <el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane label="全部" name="first" style="height:100%;">
        <template slot-scope="scope" style="height:100%;">
          <div :class="scope" style="height:100%;">
            <totalOrder :table-data="tab1" tab-id="first" :org-list="orgList" :nursing-all-list="nursingAllList" style="height:100%;" :total-items1="totalItems1" @updateData="updateData" @updateHLYData="updateHLYData" @updateOrgData="updateOrgData" @handleCurrentChange="handleCurrentChange">
      ...
      <el-tab-pane label="待派护理员/待发货/待入住/待服务" name="fourth" style="height:100%;">
        <template slot-scope="scope" style="height:100%;">
          <div :class="scope" style="height:100%;">
...
export default {
  ...
  methods: {
    apiDataTest() {
      // 长护险接口调试
      orderSearch({ 'currentPage': 1, 'numberPerPage': 10, 'sortList': this.sortList, 'orderTypes': [5, 6] }).then(res => {
        // var data1 = res.data
        this.tab1 = res.data
        this.currentPage1 = 1
        ...
        // console.log(res)
        // console.log('111')
        // orderSearch({ 'data': { 'currentPage': 1, 'numberPerPage': 10, 'filters': [{ 'key': 'Type', 'value': '4' }] }}).then(res => {
        //   console.log('111111', data1, res.data)
        //   this.tab1 = res.data.concat(data1)
        //   this.currentPage1 = 1
        //   this.totalItems1 = res.pager.totalItems
        //   console.log(res)
        // })
      })
      orderSearch({ 'currentPage': 1, 'numberPerPage': 10, 'sortList': this.sortList, 'filters': [{ 'key': 'OrderStatus', 'value': '0' }], 'orderTypes': [5, 6] }).then(res => {
        this.tab2 = res.data
        this.currentPage2 = 1
        this.totalItems2 = res.pager.totalItems
        console.log(res)
      })
      orderSearch({ 'currentPage': 1, 'numberPerPage': 10, 'sortList': this.sortList, 'filters': [{ 'key': 'OrderStatus', 'value': '1' }], 'orderTypes': [5, 6] }).then(res => {
        this.tab3 = res.data
        this.currentPage3 = 1
        this.totalItems3 = res.pager.totalItems
        console.log(res)
      })
      orderSearch({ 'currentPage': 1, 'numberPerPage': 10, 'sortList': this.sortList, 'filters': [{ 'key': 'OrderStatus', 'value': '2' }], 'orderTypes': [5, 6] }).then(res => {
        this.tab4 = res.data
        this.currentPage4 = 1
        this.totalItems4 = res.pager.totalItems
        console.log(res)
      })
      orderSearch({ 'currentPage': 1, 'numberPerPage': 10, 'sortList': this.sortList, 'filters': [{ 'key': 'OrderStatus', 'value': '3' }], 'orderTypes': [5, 6] }).then(res => {
        this.tab5 = res.data
        this.currentPage5 = 1
        this.totalItems5 = res.pager.totalItems
        console.log(res)
      })
      orderSearch({ 'currentPage': 1, 'numberPerPage': 10, 'sortList': this.sortList, 'filters': [{ 'key': 'OrderStatus', 'value': '4' }], 'orderTypes': [5, 6] }).then(res => {
        this.tab6 = res.data
        this.currentPage6 = 1
        this.totalItems6 = res.pager.totalItems
        console.log(res)
      })
      orderSearch({ 'currentPage': 1, 'numberPerPage': 10, 'sortList': this.sortList, 'filters': [{ 'key': 'OrderStatus', 'value': '5' }], 'orderTypes': [5, 6] }).then(res => {
        this.tab7 = res.data
        this.currentPage7 = 1
        this.totalItems7 = res.pager.totalItems
        console.log(res)
      })
      orderSearch({ 'currentPage': 1, 'numberPerPage': 10, 'sortList': this.sortList, 'filters': [{ 'key': 'OrderStatus', 'value': '6' }], 'orderTypes': [5, 6] }).then(res => {
        this.tab8 = res.data
        this.currentPage8 = 1
