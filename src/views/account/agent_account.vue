...
export default {
  ...
  mounted() {
    this.resourcePermission = permissionData('代理人账号')
  },
  methods: {
    loadmore() {
    ...
    addEmployee() {
      this.$router.push({ path: '/account/staff_mng/add', query: { type: 1 }})
    },
    updateEmployee() {
      if (this.multipleSelection && this.multipleSelection.length === 1) {
        this.$router.push({ path: '/account/staff_mng/update', query: { id: this.multipleSelection[0].id, type: 1 }})
      } else {
        this.$message({
          message: '请选择一条数据进行修改',
          type: 'warning'
    ...
    detail(id) {
      this.$router.push({ path: '/account/staff_mng/detail', query: { id: id, type: 1 }})
    },
