...
<template>
  ...
  <div id="scrollBox" class="ss" style="overflow:auto;height:100%">
    ...
    >
                  ...
                  <div v-if="cityOptions.length?true:false">
                    <el-checkbox-group v-model="baseForm.areaIds">
                      <el-checkbox
                        ...
                        :key="index"
                        :label="city.cityId+Separator+city.value"
                  ...
                  <el-row>
                      ...
                      <el-form-item prop="areaIds">
                        ...
                        >
                          <el-option
                            v-for="(city,index) in cityOptions"
                            :key="index"
                            :value="city.cityId+Separator+city.value"
                            :label="city.label"
                          >{{ city.label }}</el-option>
                    ...
                    <el-col :span="12" :xs="24" :sm="18">
                      <el-form-item prop="pensionAgencyId">
                        <el-select
                          v-model="baseForm.pensionAgencyId"
                          class="org_select"
                          placeholder="请选择养老机构"
                          size="small"
                          clearable
      ...
      <!-- 销售信息 -->
      <div
        v-if="this.$route.query.type==1||this.$route.query.type==5"
        class="gray_box baseView baseView2"
      >
        <div>
          ...
          <div class="item_form">
            <el-row>
              ...
              <el-col :span="12" :xs="24" :sm="8" :md="8" :lg="6">
                ...
                <el-form-item label="单位" required>
                  <el-input
                    :value="baseUnitMap[baseForm.baseUnit]"
                    disabled
                  >{{ baseUnitMap[baseForm.baseUnit] }}</el-input>
                </el-form-item>
              </el-col>
            </el-row>
          </div>
              ...
              <el-col :span="6" :xs="12" :sm="12" :md="6" :lg="6">
                <el-form-item label>
                  <el-button
                    ...
                    plain
                    size="mini"
                    @click.prevent="add_bed_info(index)"
                  >添加</el-button>
                  <el-button
                    v-if="index!=0"
                    type="primary"
                    plain
                    size="mini"
                    @click.passive="remove_bed_info(index)"
                  >删除</el-button>
                </el-form-item>
              </el-col>
            </el-row>
      ...
      <div class="gray_box baseView baseView3">
        <div>
          ...
          <div v-if="$route.query.action==2||$route.query.action==3?true:false" class="item_form">
            <el-row>
              <el-form-item label="缩略图" style="margin-bottom:0">
                <el-col>
                  <div class="avatar-uploader">
                    <div
                      class="el-upload el-upload--text listImgBox"
                      @click="function(){show=true}"
                    >
            ...
            <div class="pic_group">
              <el-row>
                <el-form-item label="图片" style="margin-bottom:0" prop="picInfo">
                  <el-col>
                    <span>图片单张大小不超过3M，图片规格700*700</span>
                  </el-col>
                  <el-col style="position:relative;over-flow:hidden">
                    ...
                    >
                      ...
                      >
                        <i
                          v-if="index==0"
                          style="position:absolute;z-index:1;top:0;left:0;display:inline-block;width:100%;background:rgb(0,0,0,0.3);height:30px;line-height:30px;color:white;font-style:normal"
                        >主图</i>
                        <i
                          v-if="!item.url"
                          slot="default"
            ...
            <el-row>
              <el-form-item label="支付方式" prop="payType">
                ...
                <el-col>
                  ...
                  <span>预约费用</span>
                  <el-input
                    v-model="baseForm.appointmentFee"
                    style="width:120px"
                    placeholder
                    size="small"
                  />
      ...
      <div
        v-show="($route.query.action=='2'||$route.query.action=='1')&&returnBotton==false"
        class="gray_box"
      >
        <div>
...
export default {
  ...
  methods: {
    ...
    redrawProductImages(resImages) {
      const productImages = [{ name: '图一', url: '', isPrimary: true }]
      ...
      if (resImages && resImages.length < 5) {
        const items = 5 - resImages.length
    ...
    // 编码格式统一
    guid() {
      return 10 + this.$moment().format('YYYYMMDDHHmmssSSSS') + parseInt(Math.random() * 9000 + 1000)
    },
    _judge() {
      ...
      if (this.$route.query.action == '2' || this.$route.query.action == '3') {
        ...
        if (this.$route.query.id) {
          searchOneGood(this.$route.query.id).then(response => {
            ...
            this.listImg = this.suoluetu.url
                ...
                filters: [
                  {
                    key: 'CityCode',
                    filterType: 0,
                    value: data.productAreas && data.productAreas.length ? data.productAreas[0].areaCode : null
                  }
                ],
                numberPerPage: 20,
                currentPage: 1,
    ...
    fnUploadRequestTest(option) {
      ...
      client.multipartUpload(fileNames, files, {
        progress: function(p) {
          const e = {}
          e.percent = Math.floor(p * 100)
          option.onProgress(e)
        ...
        if (result.res.status === 200) {
          ...
          quill.insertEmbed(length, 'image', 'http://ecare-test.oss-cn-hangzhou.aliyuncs.com/' + result.name)
          // 调整光标到最后
