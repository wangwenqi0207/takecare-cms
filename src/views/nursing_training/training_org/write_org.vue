  ...
  <div id="scrollBox" class="add_org" style="overflow:auto;height:100%">
    ...
    >
        ...
        <div class="base_info_main">
          ...
          <div class="base_info_main_one">
            <el-form-item label="服务城市" required prop="city">
              <el-select v-model="ruleForm.city" clearable placeholder="请选择" style="margin-right: 209px;">
                <el-option
      ...
      <div class="org_submit">
        ...
        <el-button type="primary" size="medium" class="org_submit_btn" @click="update_submit()">提交</el-button>
      </div>
    </el-form>
  </div>
</template>
...
import { orgUpdate, orgInfo } from '@/api/organization'
...
export default {
  ...
  data() {
    var picInfoValidate2 = (rule, value, callback) => {
      ...
      const arr = this.Imglist.some((item) => {
        return item.url !== ''
      })
      if (arr && this.Imglist[0].url !== '') {
        callback()
      } else {
  ...
  methods: {
    ...
    hintGetFun() {
      orgInfo({ id: this.$route.query.id }).then(res => {
        console.log(res)
        if (res) {
          var result = res
          this.ruleForm.orgImgDetail = result.orgImgDetail
          this.ruleForm = result
          // this.ruleForm.orgClass = String(result.orgClass).split(',')
          // this.ruleForm.fixedPointMechanism = String(result.fixedPointMechanism).split(',')

          if (result.organizationStatus) {
            this.ruleForm.organizationStatus = true
          } else {
            this.ruleForm.organizationStatus = false
          }
          var IMGLists = result.organizationImages
          // console.log(result.organizationImages)
          IMGLists.length = 5
          this.ruleForm.listPicture = IMGLists[0]
          this.Imglist = [{
            name: '图1',
            url: IMGLists[1] ? IMGLists[1] : '',
            isPrimary: true
          },
          {
            name: '图2',
            url: IMGLists[2] ? IMGLists[2] : '',
            isPrimary: false
          },
          {
            name: '图3',
            url: IMGLists[3] ? IMGLists[3] : '',
            isPrimary: false
          },
          {
            name: '图4',
            url: IMGLists[4] ? IMGLists[4] : '',
            isPrimary: false
          }]
    ...
    fnUploadRequest2(option) {
      ...
      client.multipartUpload(fileNames, option.file, {
        ...
        if (val.res.statusCode === 200) {
          ...
          this.Imglist[this.uploadImageIndex].url = 'http://ecare-test.oss-cn-hangzhou.aliyuncs.com/' + val.name
          // var tempArray = this.ruleForm.Imglist;
          // tempArray[this.uploadImageIndex].url = 'http://ecare-test.oss-cn-hangzhou.aliyuncs.com/' + val.name;
          // this.ruleForm.Imglist = tempArray;
          // Vue.set(this.Imglist, this.uploadImageIndex, {
          //   name: '图'+this.uploadImageIndex,
          //   url:'http://ecare-test.oss-cn-hangzhou.aliyuncs.com/' + val.name,
          //   isPrimary: this.uploadImageIndex == 0?true:false
          // })
          // this.array.$set(indexOfItem, newValue)
          // this.ruleForm.Imglist[this.uploadImageIndex] = {
          //   name: '图'+this.uploadImageIndex+1,
          //   url:'http://ecare-test.oss-cn-hangzhou.aliyuncs.com/' + val.name,
          //   isPrimary: this.uploadImageIndex == 0?true:false
          // }
          console.log('888888', this.Imglist)
