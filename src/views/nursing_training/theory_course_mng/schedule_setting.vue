    ...
    <div v-if="isNative">
      <div class="search_bar" style="margin-bottom:20px;">
        <div class="search_bar_left">
          <span style="font-size:26px;font-weigth:700;margin-right:15px;">护理理论知识课程</span>
          <span>义乌工商职业技术学校</span>
        </div>
      ...
      <div class="tableList" style="height:83%">
        ...
        >
          ...
          <el-table-column label="期数" prop="name" />
...
export default {
  ...
  created() {
    if (this.$route.path === '/nursing_training/theory_course_mng/schedule_setting') {
      this.isNative = true
    } else {
      this.isNative = false
    }
  ...
  methods: {
    ...
    handleEdit(id) {
      this.$router.push({
        path: '/nursing_training/theory_course_mng/schedule_setting/schedule_detail',
        query: {
          id: id
        }
      })
