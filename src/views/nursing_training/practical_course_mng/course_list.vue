  ...
  <div class="comment_box">
          ...
          <el-form-item label class="searchFormItem">
            <el-input v-model="searchForm.keyWord" class="searchInput" placeholder="请输入内容" clearable>
              <el-select slot="prepend" v-model="searchForm.searchItem" placeholder="请选择" class="input-with-select" style="width:120px">
                <el-option label="考核名称" value="1" />
                <el-option label="考核机构" value="2" />
              </el-select>
    ...
    <div class="tableList" style="height:83%">
      ...
      >
        ...
        <el-table-column label="考核机构" prop="idCard" />
        <el-table-column label="考核地址" prop="idCard" />
...
export default {
  ...
  methods: {
    ...
    _addData() {
      this.$router.push('/nursing_training/practical_course_mng/add_practical_course')
    },
    handleEdit(id) {
      this.$router.push({
        path: '/nursing_training/practical_course_mng/look_practical_course',
        query: {
          id: id
        }
      })
    ...
    _Scheduling(id) {
      this.$router.push({
        path: '/nursing_training/practical_course_mng/schedule_setting',
        query: {
          id: id
        }
      })
    ...
    updateData(id) {
      this.$router.push({
        path: '/nursing_training/practical_course_mng/update_practical_course',
        query: {
          id: id
        }
      })
    ...
    removeData(id) {
      this.$confirm('删除后不可恢复，是否确认删除此考核?', '提示', {
