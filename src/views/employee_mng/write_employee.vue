        ...
        <el-col :span="20">
          <el-form ref="ruleForm" label-width="160px" :model="ruleForm" :rules="rules">
            <el-row id="zhqx1">
              ...
              <span style="font-size: 20px;color: #3e8dff;">人员修改</span>
            </el-row>
            <el-row>
              <el-col :span="9">
            ...
            <el-row>
              <el-col :span="9">
                <el-form-item label="姓" required prop="lastName">
                  <el-input v-if="ruleForm.identityCardCertificationStatus==1" v-model="ruleForm.lastName" disabled />
                  <el-input v-else v-model="ruleForm.lastName" />
                </el-form-item>
              </el-col>
              <el-col :span="9">
                <el-form-item label="名" required prop="firstName">
                  <el-input v-if="ruleForm.identityCardCertificationStatus==1" v-model="ruleForm.firstName" disabled />
                  <el-input v-else v-model="ruleForm.firstName" />
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="9">
                <el-form-item label="性别" required prop="sex">
                  <el-radio v-if="ruleForm.identityCardCertificationStatus==1" v-model="ruleForm.sex" label="0" style="margin-left:20px;" disabled>男</el-radio>
                  <el-radio v-else v-model="ruleForm.sex" label="0" style="margin-left:20px;">男</el-radio>
                  <el-radio v-if="ruleForm.identityCardCertificationStatus==1" v-model="ruleForm.sex" label="1" disabled>女</el-radio>
                  <el-radio v-else v-model="ruleForm.sex" label="1">女</el-radio>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="9">
                <el-form-item label="身份证" required prop="identityCard">
                  <el-input v-if="ruleForm.identityCardCertificationStatus==1" v-model="ruleForm.identityCard" disabled />
                  <el-input v-else v-model="ruleForm.identityCard" />
                </el-form-item>
              </el-col>
              <el-col :span="9">
...
<script>
import { staffUpdate, staffInfo } from '@/api/nursingStaff'
...
export default {
  ...
  data() {
    return {
      ...
      // 记录原来的姓名及身份证号，以判断是否需要再次调用实名认证接口
      firstNameYL: '',
      lastNameYL: '',
      sexYL: '',
      identityCardYL: ''
    ...
    getStaffInfo() {
      staffInfo(this.$route.query.id).then(res => {
        if (res) {
          ...
          this.firstNameYL = res.firstName
          this.lastNameYL = res.lastName
          this.sexYL = res.sex + ''
          this.identityCardYL = res.identityCard
          if (res.staffLogs) {
    ...
    submitForm: Debounce(function(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          // console.log(this.ruleForm.areaCode,this.ruleForm.areaName)
          ...
          staffUpdate(this.ruleForm).then(res => {
            if (res) {
              this.$message({
                type: 'success',
                message: '修改成功'
              })
              if (this.firstNameYL != this.ruleForm.firstName || this.lastNameYL != this.ruleForm.lastName || this.identityCardYL != this.ruleForm.identityCard ||
              this.sexYL != this.ruleForm.sex) {
...
// 图片上传框的格式
#add_employee1 .avatar-uploader .el-upload {
...
#add_employee1 .avatar-uploader .el-upload:hover {
    border-color: #409EFF;
  }
#add_employee1 .avatar-uploader-icon {
    font-size: 28px;
    color: #8c939d;
    width: 100%;
    height: 120px;
