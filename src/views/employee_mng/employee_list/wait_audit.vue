  ...
  >
    <div class="search_bar" style="padding:0px;margin:0px;">
      ...
      >
        ...
        <el-button v-if="resourcePermission['审核通过']" type="primary" style="float:right;margin-left:10px;" size="medium" @click="reject">审核不通过</el-button>
        <el-button v-if="resourcePermission['审核未通过']" type="primary" style="float:right;" size="medium" @click="pass">审核通过</el-button>
    ...
    <!-- 确认退单 -->
    <el-dialog title="审核不通过" :visible.sync="dialogFormVisible" width="500px">
      <el-form ref="form" :model="form" :rules="rules">
        <el-form-item label="未通过原因：" :label-width="formLabelWidth" prop="reason">
          <!-- <el-select v-model="formQR.name" style="width:80%;" >
            <el-option label="通过" value="shanghai" />
            <el-option label="未通过" value="beijing" />
          </el-select> -->
          <el-input v-model.trim="form.reason" autocomplete="off" style="width:80%;" />
        </el-form-item>
      </el-form>
...
<script>
import { staffSearch, approve, refusetopass } from '@/api/nursingStaff'
...
export default {
  ...
  mounted() {
    this.resourcePermission = permissionData('待审核护理人员')
  },
  methods: {
    loadmore() {
    ...
    handleCommand(command) {
      switch (command.flag) {
        case 1:
          ...
          break
        case 2:
          this.all_org = command.label
          this.all_orgValue = command.value
          break
        case 3:
    ...
    onSubmit() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          refusetopass({ id: staffId, refusalOfReason: this.form.reason }).then(res => {
            if (res) {
              this.searchData()
              this.$message({
                message: '审核操作成功',
    ...
    pass() {
      if (this.multipleSelection && this.multipleSelection.length === 1) {
        this.$confirm('确认审核通过该条数据吗?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          var staffId = this.multipleSelection[0].id
          approve(staffId).then(res => {
            if (res) {
              this.searchData()
              this.$message({
                message: '审核操作成功',
    ...
    searchData() {
      console.log(this.all_typeValue, this.all_orgValue, this.all_statusValue, this.org_name, this.org_nameType)
      this.filterData = []
      var filterData = []
      if (this.org_name) {
