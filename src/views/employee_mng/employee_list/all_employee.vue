  ...
  >
    <div class="search_bar" style="padding:0px;margin:0px;">
      ...
      >
        ...
        <el-button v-if="resourcePermission['删除']" type="primary" style="float:right;margin-left:10px;" size="medium" @click="remove">删除</el-button>
        <el-button v-if="resourcePermission['修改']" type="primary" style="float:right;" size="medium" @click="update">修改</el-button>
        <el-button v-if="resourcePermission['新增']" type="primary" style="float:right;" size="medium" @click="add">新增人员</el-button>
...
<script>
import { staffSearch, staffDelete } from '@/api/nursingStaff'
...
export default {
  ...
  mounted() {
    this.resourcePermission = permissionData('全部护理人员')
  },
  methods: {
    loadmore() {
    ...
    update() {
      if (this.multipleSelection && this.multipleSelection.length === 1) {
        ...
        // console.log(org_id)
        this.$router.push({
          path: '/employee_mng/care_employee/write_employee',
          query: {
            id: id
          }
        })
    ...
    remove() {
      if (this.multipleSelection && this.multipleSelection.length === 1) {
        this.$confirm('确认删除该条数据吗?', '提示', {
          ...
          var staffId = this.multipleSelection[0].id
          staffDelete({ id: staffId }).then(res => {
            if (res) {
              this.currentPage = 1
              this.searchData()
              this.$message({
    ...
    // 新增
    add() {
      this.$router.push('/employee_mng/care_employee/add_employee')
    },
